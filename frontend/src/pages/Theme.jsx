...
export default function Styling() {
  ...
  const handleClearTheme = async () => {
    try {
      ...
      const { data } = await api.post("/portfolio/update-theme", {
    ...
    <div className="w-full mx-auto p-6 space-y-8">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-amber-800">
          <span className="font-semibold">💡 Tip:</span> It's recommended to use
          the colors that come with templates. However, you can still customize
          them here if needed.
        </p>
      </div>
      {theme && Object.values(theme).some(val => val) && (
        <div className="flex justify-between items-center">
          <Button variant="outline" onClick={handleClearTheme}>
            Clear Theme
          </Button>
          <Button onClick={handleUpdateTheme} disabled={loading}>
